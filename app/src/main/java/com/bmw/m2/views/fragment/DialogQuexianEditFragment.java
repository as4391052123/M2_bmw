...
import com.bmw.m2.model.PictureQueXianInfo;
import com.bmw.m2.model.QueXianInfo;
import com.bmw.m2.utils.DbHelper;
import com.bmw.m2.utils.PullXmlParseUtil;
import com.bmw.m2.views.adapter.SpinnerAdapter;
import com.bmw.m2.views.dialog.ClockShow_Dialog;
...
public class DialogQuexianEditFragment extends DialogFragment {
                    ...
                    public void run() {
                        ...
                        if(TextUtils.isEmpty(mStyleSp.getText().toString()) || mStyleSp.getText().equals("请选择")){
                            handler.post(new Runnable() {
                                @Override
                                public void run() {
                                    VLCApplication.toast(getResources().getString(R.string.chooseQuexianSort));
                                }
                            });
                            return;
                        }
    ...
    private void initAdapter() {
        ...
        mQuexianNameAdapter = new SpinnerAdapter(getActivity());
        mGradeAdapter = new SpinnerAdapter(getActivity());
        mStyleAdapter = new SpinnerAdapter(getActivity());
    ...
    private void setEditText(boolean b) {
        ...
        if (!b) {
            ...
            mNameTv.setTextColor(getActivity().getResources().getColor(R.color.colorBase));
            mClockTv.setTextColor(getActivity().getResources().getColor(R.color.colorBase));
            mGradeTv.setTextColor(getActivity().getResources().getColor(R.color.colorBase));
            mLengthTv.setTextColor(getActivity().getResources().getColor(R.color.colorBase));
            mDetailTv.setTextColor(getActivity().getResources().getColor(R.color.colorBase));
        }
    }
