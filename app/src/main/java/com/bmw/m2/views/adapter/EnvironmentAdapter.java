...
import com.bmw.m2.views.dialog.EnvironmentDialog;
...
public class EnvironmentAdapter extends RecyclerView.Adapter<EnvironmentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        final Environment environment = list.get(position);
...
//                if (e_name.equals("气压")) {
                    min_current = environment.getMin_num();
                    max_current = environment.getMax_num();
                    e_unit = environment.getUnit();
//                }
                    ...
                    public void setDate(String min, String max) {  //传回更改数据
                        if (min.equals("") || max.equals("")) {
                            ...
                            if (min_num < 0 || min_num > max_num) {
                                ...
                                resetMaxAMin(list.get(position).getName(), min_num, max_num);  //更改最大最小值
    ...
    private void resetMaxAMin(String name, float min_num, float max_num) {
        ...
        editor.putFloat(name+"_min", min_num);
        editor.putFloat(name+"_max", max_num);
        editor.commit();
    }
