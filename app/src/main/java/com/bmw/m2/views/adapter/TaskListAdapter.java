...
public class TaskListAdapter extends RecyclerView.Adapter<TaskListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (!isClicks.get(position)) {
            holder.container.setBackgroundColor(context.getResources().getColor(R.color.colorLucency));
            setTextColorYellow(true, holder);
        } else {
            holder.container.setBackgroundColor(context.getResources().getColor(R.color.colorBase));
    ...
    private void setTextColorYellow(boolean b, ViewHolder holder) {
        int colorId = 0;
        if (b) {
            colorId = context.getResources().getColor(R.color.colorBase);
        } else {
    ...
    public void setChooseByTaskId(int id){
        log("recordHeadIdFind：comeId = "+id);
        if(id <0 )
            return;
        for(int i=0;i<list.size();i++){
            log("recordHeadIdFind：findId "+i+" = "+list.get(i).getId());
